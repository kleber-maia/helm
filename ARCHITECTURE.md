...
Helm is a visual Git client for macOS written in Swift, leveraging **libgit2** for Git operations and a hybrid **AppKit/SwiftUI** UI layer. The architecture emphasizes:
...
Helm extensively uses protocols to abstract functionality:
...
┌─────────────▼───────────────────────────┐
│       Repository Layer                   │
│  - HelmRepository (libgit2 wrapper)        │
...
### **1. Repository Layer** (`Helm/Repository/`)
...
#### Related Files:
- `HelmRepository.swift` - Main repository implementation
- `HelmRepository+Commands.swift` - Git commands (checkout, stage, etc.)
...
### **2. Controller Layer** (`Helm/Repository/`, `Helm/Document/`)
...
**Implemented by:** `HelmWindowController`
...
#### **Operation Controllers** (`Helm/Operations/`)
...
RepoDocument (NSDocument)
    └── HelmWindowController (main window)
...
- Detects file changes outside of Helm
...
`HelmRepository` abstracts libgit2, providing:
...
3. repository.stage(file: path)
   ↓
4. HelmRepository.performWriting { 
...
1. User clicks commit in HistoryTableController
   ↓
2. HelmWindowController.select(oid: GitOID)
   ↓
...
Helm uses the `@Faked` macro and manual fake classes for testing:
...
2. **Implement in extension**:
   ```swift
   extension HelmRepository: MyFeature {
     func doSomething() throws {
       try performWriting {
         // libgit2 calls
       }
...
When contributing to Helm, keep these architecture principles in mind:
