# GitHub Copilot / Copilot Chat instructions for Helm

Short purpose
-------------
Helm is a native macOS Git client written primarily in Swift and Cocoa (AppKit) with a small Objective-C/Bridging-Header surface and a bundled libgit2 C dependency. Use these instructions to help produce focused, repository-aware suggestions, edits, and tests that fit the project's conventions.
...
------------------------------
- Swift (macOS AppKit)
- Objective-C headers / bridging (see `Helm-Bridging-Header.h`)
- C (libgit2) included under `libgit2/` and `libgit2-mac.a`
- Tests use XCTest in the `HelmTests/` target

Environment & build notes
-------------------------
- This is a macOS app. Prefer an up-to-date Xcode for local builds.
- Typical local workflows:
  - Open the workspace/project in Xcode and build/run the `Helm` scheme.
  - Example CLI build (may need to be adapted to local Xcode version and signing):
    - Build: `xcodebuild -project Helm.xcodeproj -scheme Helm -configuration Debug build`
    - Run tests: `xcodebuild -project Helm.xcodeproj -scheme Helm -configuration Debug test`
...
-----------------------------
- `Helm/` — main app source. Look here first for UI, controllers, models.
- `Helm.xcodeproj/` — Xcode project. `project.pbxproj` contains build settings.
- `Helm-Bridging-Header.h` — Objective-C/Swift bridging declarations.
- `libgit2/`, `libgit2-mac.a`, `build_libgit2.sh` — native libgit2 dependency and helper.
- `HelmTests/` — unit tests and test helpers (use these as references and to add tests).
...
-------------------------
- Follow the existing Swift style in the `Helm/` tree and as indicated by .swiftlint.yml. If unsure, mirror adjacent files in the same folder.
...
- When adding tests: use `HelmTests/` and XCTest; include both a happy-path test and at least one edge-case test where practical.

Common tasks & how to run them
------------------------------
- Build in Xcode: open `Helm.xcodeproj`, select the `Helm` scheme, and click Run.
- CLI build: `xcodebuild -project Helm.xcodeproj -scheme Helm -configuration Debug build`
- Run tests in Xcode: select the `HelmTests` scheme / test target and run tests.
- CLI tests: `xcodebuild -project Helm.xcodeproj -scheme Helm -configuration Debug test`
...
- When adding or changing public API, include a small unit test in `HelmTests/` demonstrating the behavior.
- Always provide commands to run to verify changes (build and tests).

Example prompts (good)
----------------------
- "Implement an XCT test in `HelmTests/SidebarDataModelTest.swift` that covers empty repo handling and assert the model returns an empty list." — Answer should include a short checklist, the unit test file edits, and the xcodebuild test command.
...
- Test: open `Helm` in Xcode and run `HelmTests/SidebarDataModelTest` or run `xcodebuild test -project Helm.xcodeproj -scheme Helm -configuration Debug test`
- Files: `Sidebar/SidebarController.swift`, `HelmTests/SidebarDataModelTest.swift`

Small housekeeping & conventions
-------------------------------
- If you update Swift APIs or add dependency code, also add or update unit tests in `HelmTests/`.
