...
{
  static let amend = NSTouchBarItem.Identifier("group.soft.HelmIDE.amend")
  static let commit = NSTouchBarItem.Identifier("group.soft.HelmIDE.commit")
}
...
{
  typealias Repository = BasicRepository & CommitStorage & CommitReferencing & FileContents

  private weak var repo: (any Repository)!
  {
    didSet
    {
      if let controller = (repo as? HelmRepository)?.controller {
        Task {
  ...
  func commitMessageTemplate() -> String?
  {
    ...
    let expanded = NSString(string: templatePath).expandingTildeInPath
    let url: URL
    if expanded.hasPrefix("/") {
      url = URL(fileURLWithPath: expanded)
    }
    else {
      guard let repoURL = repo?.repoURL
      else { return nil }
      url = repoURL.appendingPathComponent(expanded)
    }
  ...
  func commit(_ sender: NSButton)
  {
    do {
      ...
      if let controller =
          (repo as? HelmRepository)?.controller {
        controller.indexChanged()
        controller.refsChanged()
      }
      resetMessage()
      resetAmend()
    }
