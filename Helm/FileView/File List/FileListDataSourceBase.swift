...
class FileListDataSourceBase: NSObject
{
  ...
  /// When `true`, this data source belongs to the review panel and only
  /// reloads for `StagingSelection`. When `false` (history panel),
  /// staging selections are ignored.
          ...
          repoUIController.selectionPublisher
            .sink {
              ...
              guard let self = self,
                    ...
                    self.repoUIController != nil
              else { return }
              let isStaging = selection is StagingSelection
              guard isStaging == self.isReviewPanel
              else { return }
  ...
  {
    guard isReviewPanel,
          repoUIController?.selection is StagingSelection
    else { return }
    (self as! FileListDataSource).reload()
  }
}
