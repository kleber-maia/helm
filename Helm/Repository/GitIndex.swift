...
public protocol StagingIndex
{
  ...
  associatedtype ConflictEntry: Helm.ConflictEntry<Entry>
...
{
  typealias ConflictEntry = GitConflictEntry
...
extension GitIndex
{
  // Unchecked because git_index_entry isn't technically Sendable but it
  ...
  struct GitConflictEntry: Helm.ConflictEntry
  {
    typealias Entry = GitIndex.Entry
  ...
  class ConflictIterator: IteratorProtocol
  {
    ...
    func next() -> GitConflictEntry?
    {
      ...
      return GitConflictEntry(ancestor: Entry(gitEntry: finalAncestor),
