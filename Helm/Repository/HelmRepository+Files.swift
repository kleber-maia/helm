...
{
  case commit(any Helm.Commit)
  case index
  case workspace
}

// MARK: FileContents
extension HelmRepository: FileContents
{
  ...
  public func isTextFile(_ path: String, context: FileContext) -> Bool
  {
    ...
    if HelmRepository.textNames.contains(name) {
      return true
    }
    if HelmRepository.isTextExtension(name) {
      return true
    }
...
// MARK: FileDiffing
extension HelmRepository: FileDiffing
{
    ...
    do {
      ...
      return performReading {
        if let index = GitIndex(repository: gitRepo),
           ...
           let indexBlob = GitBlob(repository: gitRepo,
                                   oid: indexEntry.oid) {
          return .diff(PatchMaker(
              from: PatchMaker.SourceType(indexBlob),
...
public extension HelmRepository
{
  ...
  class StatusCollection: BidirectionalCollection
  {
    ...
    init(repo: HelmRepository, head: (any Helm.Commit)?)
    {
      let (list, ownedTree) = repo.performReading {
        () -> (OpaquePointer?, OpaquePointer?) in
        let headTree = ((head as? GitCommit)?.tree)?.tree
        var options = git_status_options()

        git_status_init_options(
            &options,
            UInt32(GIT_STATUS_OPTIONS_VERSION))
        options.flags =
            GIT_STATUS_OPT_INCLUDE_UNTRACKED.rawValue |
            GIT_STATUS_OPT_RECURSE_UNTRACKED_DIRS.rawValue

        var emptyTree: OpaquePointer?

        if let tree = headTree {
          options.baseline = tree
        }
        else {
          emptyTree = StatusCollection.emptyTree(repo: repo)
          options.baseline = emptyTree
        }

        let statusList = try? OpaquePointer.from {
          git_status_list_new(&$0, repo.gitRepo, &options)
        }
        return (statusList, emptyTree)
      }
    ...
    convenience init(repo: HelmRepository)
    {
      let head = repo.headOID
                     .flatMap { repo.commit(forOID: $0) }
      self.init(repo: repo, head: head)
    }
  
    static func emptyTree(repo: HelmRepository) -> OpaquePointer?
    {
