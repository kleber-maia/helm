...
extension HelmRepository: FileStatusDetection
{
                     ...
                     amend: Bool = false) -> [FileChange]
  {
    ...
    return performReading {
      guard let statusList = GitStatusList(
          repository: self, show: show,
          options: options)
      else { return [] }

      return statusList.compactMap {
        (entry) in
        let delta = (show == .indexOnly)
            ? entry.headToIndex : entry.indexToWorkdir

        return delta.map {
          FileChange(path: $0.newFile.filePath,
                     oldPath: $0.oldFile.filePath,
                     change: $0.deltaStatus)
        }
      }
...
extension HelmRepository: FileStaging
{
  ...
  public func unstageAllFiles() throws
  {
    ...
    if let headOID = headReference?.resolve()?.targetOID as? GitOID {
