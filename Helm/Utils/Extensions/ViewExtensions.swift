...
extension NSView
{
  ...
  func addBorderLines()
  {
    for edge in [NSLayoutConstraint.Attribute.top,
...
extension NSSplitView
{
  ...
  func equalizeSubviews(atDivider index: Int)
  {
    let left = subviews[index]
    let right = subviews[index + 1]
    let equalSize = isVertical
        ? (left.frame.width + right.frame.width) / 2
        : (left.frame.height + right.frame.height) / 2

    animate(position: equalSize, ofDividerAtIndex: index)
  }
}
